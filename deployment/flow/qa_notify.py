"""Cron flow-qa-notify — processa ``flow:qa-waiting`` (aviso ao QA).

Notifica o QA de que há trabalho aguardando testes em HML. Não despacha
sessão — apenas avisa o papel correto.
...
def run(ctx: object) -> None:
    """Cron flow-qa-notify — processa flow:qa-waiting (notifica QA)."""
    run_stage(ctx, FlowStage.QA_NOTIFY)
