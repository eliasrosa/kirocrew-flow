"""Cron flow-conflito — processa ``flow:merge-conflict``.

Despacha sessões one-shot de resolução de conflito (rebase na branch
``feat/issue-<number>`` existente, nunca abre PR nova). Intervalo de 300s.
...
def run(ctx: object) -> None:
    """Cron flow-conflito — processa flow:merge-conflict."""
    run_stage(ctx, FlowStage.CONFLITO)
