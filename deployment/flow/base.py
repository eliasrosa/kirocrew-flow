...
#
# Este módulo vive em ``deployment/flow/base.py``. O layout é:
#
#   <repo>/deployment/flow/base.py   ← este arquivo (_MODULE_DIR)
#   <repo>/deployment/               ← pacote deployment (compat stub aqui)
#   <repo>/flow/                     ← pacote hexagonal top-level
#
# ``_HERE`` continua apontando para o diretório ``deployment/`` (não
# ``deployment/flow/``) para preservar a semântica histórica de
# ``_CONFIG_CANDIDATES`` e ``deployment.version`` — os testes que fazem
# ``mock.patch("deployment.deployment._HERE", ...)`` dependem disso.
...
class FlowStage(StrEnum):
    """Estágios da esteira, um por cron.

    Cada membro é o identificador do estágio usado por ``_run_stage`` e
    ``_STAGE_ACTIONS``. Os quatro primeiros são estágios de dispatch/merge; os
    três últimos (``REWORK``, ``QA_NOTIFY``, ``QA_REFUSED``) são estágios de
    notificação (gates humanos / avisos) que NUNCA despacham sessões — apenas
    avisam o papel correto.

    Os valores string batem com as chaves de ``stage_models`` na config.
    """
    ...
    MERGE      = "merge"
    CONFLITO   = "conflito"
    # Estágios de notificação (issue #195) — gates humanos e avisos.
    REWORK     = "rework"
    QA_NOTIFY  = "qa_notify"
    QA_REFUSED = "qa_refused"


# Aliases string retrocompatíveis — os testes importam estes nomes de
# deployment.deployment. Mantidos como str (== FlowStage.<X>) para não quebrar
# usos posicionais existentes.
_STAGE_DEV      = FlowStage.DEV
_STAGE_REVIEWER = FlowStage.REVIEWER
_STAGE_MERGE    = FlowStage.MERGE
_STAGE_CONFLITO = FlowStage.CONFLITO
_STAGE_REWORK     = FlowStage.REWORK
_STAGE_QA_NOTIFY  = FlowStage.QA_NOTIFY
_STAGE_QA_REFUSED = FlowStage.QA_REFUSED
...
_STAGE_ACTIONS: dict[str, frozenset[str]] = {
...
# ── Estágios de notificação (issue #195) ──────────────────────────────────
# Estes crons NÃO despacham sessões. Eles filtram decisões NOTIFY_HUMAN
# emitidas pelo executor para um ÚNICO estado e notificam o papel correto.
#
# NOTIFY_HUMAN é uma ActionKind transversal (ver _TRANSVERSAL em
# test_deployment_integration.py) — por isso NÃO entra em _STAGE_ACTIONS
# (senão violaria test_estagios_sem_overlap, que exige que cada ActionKind
# pertença a no máximo um estágio). O roteamento destes crons é por ESTADO,
# não por ActionKind.
#
# Regra inviolável (fluxo.md): review-refused e qa-refused são gates humanos —
# NUNCA despacham automaticamente, apenas notificam. qa-waiting apenas avisa QA.
#
# Cada entrada mapeia o estágio → o valor de State.value que ele processa.
_NOTIFY_STAGE_STATES: dict[str, str] = {
    _STAGE_REWORK:     "flow:review-refused",  # gate humano — notifica TL
    _STAGE_QA_NOTIFY:  "flow:qa-waiting",       # aviso — notifica QA
    _STAGE_QA_REFUSED: "flow:qa-refused",       # gate humano — notifica TL (+dev)
}
...
def _run_notify_stage(ctx: object, stage: str) -> None:
    """Ciclo de um estágio de notificação (issue #195).

    Estes crons — ``rework`` (flow:review-refused), ``qa_notify``
    (flow:qa-waiting) e ``qa_refused`` (flow:qa-refused) — NÃO despacham
    sessões. Cada um escaneia, filtra as decisões ``NOTIFY_HUMAN`` do executor
    cujo estado bate com o estado-alvo do estágio, e notifica o papel correto
    via ``_notify_human_actions`` (mesma lógica de anti-spam e papéis do run
    monolítico).

    Gates humanos (rework/qa_refused) são invioláveis: aqui NUNCA há dispatch
    automático — apenas notificação. qa_notify apenas avisa o QA.

    O filtro é por ESTADO (``_NOTIFY_STAGE_STATES[stage]``), não por ActionKind,
    porque ``NOTIFY_HUMAN`` é transversal e compartilhada entre os três estágios.
    """
    _check_installed_version(ctx)
    cfg = _load_config()

    target_state = _NOTIFY_STAGE_STATES[stage]
    ...
    # ── Carrega a SquadConfig (mesma lógica do _run_stage) ────────────────
    from flow.config.squad import SquadConfig, SquadConfigError, load_squad
    ...
    needs_human: list = []
    for result in scan_results:
        if result.current_state is None or result.current_state.value != target_state:
            continue
        ...
        # Gate inviolável: estes estágios só notificam. Qualquer coisa que não
        # seja NOTIFY_HUMAN é ignorada (nunca dispatch).
        if decision.action is ActionKind.NOTIFY_HUMAN:
            needs_human.append((result, decision, state_comment))
        else:
            logger.debug(
                "deployment[%s]: action %s ignorada (estágio de notificação só notifica)",
    ...
    # qa_refused é gate humano que envolve TL E dev (fluxo.md): o executor
    # notifica o TL; aqui adicionamos um aviso extra ao dev para que ele saiba
    # que precisa retomar a issue. Continua sendo NOTIFICAÇÃO — sem dispatch.
    if stage == _STAGE_QA_REFUSED:
        vm = f" (voice_maybe chat_id {chat_id}, intent auto)" if chat_id else ""
        linhas = "\n".join(
            f"  - {r.item.key}: {r.item.title} — {d.reason}"
            for r, d, _sc in needs_human
        )
        ctx.notify(  # type: ignore[attr-defined]
            f"KiroCrew Flow [qa_refused]: QA reprovou — gate humano. "
            f"Dev deve retomar a issue (mover manualmente).{vm}\n{linhas}"
        )


def run_stage(ctx: object, stage: str) -> None:
    """Entrypoint público usado pelos arquivos por cron em ``deployment/flow/``.

    Cada ``deployment/flow/<cron>.py`` expõe ``run(ctx)`` que chama
    ``run_stage(ctx, FlowStage.<STAGE>)``. Delega para ``_run_stage`` — o nome
    ``run_stage`` é o contrato estável documentado na issue; ``_run_stage``
    permanece por retrocompatibilidade com os testes que o importam.
    """
...
def _run_stage(ctx: object, stage: str) -> None:
    ...
    Estágios de notificação (rework/qa_notify/qa_refused) são roteados por
    ESTADO via ``_NOTIFY_STAGE_STATES`` e apenas notificam o papel correto —
    nunca despacham sessão (gates humanos são invioláveis).

    Args:
        ctx:   contexto do cron do Kiro Crew
        stage: um dos valores de FlowStage (dev/reviewer/merge/conflito/
               rework/qa_notify/qa_refused)
    """
    if stage in _NOTIFY_STAGE_STATES:
        _run_notify_stage(ctx, stage)
        return
...
def run_rework(ctx: object) -> None:
    """Entrypoint do cron de re-trabalho pós-review (issue #195).

    Processa issues em ``flow:review-refused`` — **gate humano inviolável**.
    NUNCA despacha automaticamente: apenas notifica o TL de que o reviewer
    reprovou e o fluxo aguarda decisão manual.
    ...
    Registro (uma vez):
        cron_add(name="flow-rework",
                 ...
                 every=300)
    """
    _run_stage(ctx, _STAGE_REWORK)


def run_qa_notify(ctx: object) -> None:
    """Entrypoint do cron de aviso de QA (issue #195).

    Processa issues em ``flow:qa-waiting`` e notifica o QA de que há trabalho
    aguardando testes em HML. Não despacha sessão — apenas avisa.
    ...
    Registro (uma vez):
        cron_add(name="flow-qa-notify",
                 ...
                 every=300)
    """
    _run_stage(ctx, _STAGE_QA_NOTIFY)


def run_qa_refused(ctx: object) -> None:
    """Entrypoint do cron de QA reprovado (issue #195).

    Processa issues em ``flow:qa-refused`` — **gate humano inviolável**.
    NUNCA despacha automaticamente: notifica o TL (via executor) e também o
    dev de que o QA reprovou e o fluxo aguarda decisão manual.
    ...
    Registro (uma vez):
        cron_add(name="flow-qa-refused",
                 ...
                 every=300)
    """
    _run_stage(ctx, _STAGE_QA_REFUSED)
