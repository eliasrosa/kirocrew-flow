"""Cron flow-qa-refused — processa ``flow:qa-refused`` (gate humano).

**Gate humano inviolável (fluxo.md).** Quando o QA reprova, este cron apenas
NOTIFICA o TL (via executor) e o dev — NUNCA despacha uma sessão
automaticamente. O fluxo para e aguarda decisão manual.
...
def run(ctx: object) -> None:
    """Cron flow-qa-refused — processa flow:qa-refused (notifica TL+dev, não despacha)."""
    run_stage(ctx, FlowStage.QA_REFUSED)
