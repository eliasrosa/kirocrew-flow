"""Cron flow-reviewer — processa ``flow:review-waiting`` (sem ``flow:reviewed``).

Despacha sessões one-shot do kiro-reviewer (code review). Intervalo de 300s.
...
def run(ctx: object) -> None:
    """Cron flow-reviewer — processa flow:review-waiting."""
    run_stage(ctx, FlowStage.REVIEWER)
