...
#
# Cobertura completa das ActionKinds de dispatch/mutação: cada ação emitida
# pelo executor que exige efeito colateral (dispatch de sessão ou mutação de
# label) DEVE aparecer em algum estágio, ou o modo por-estágio a descarta
# silenciosamente (regressão #87). O guard test test_stage_actions_parity.py
# quebra se uma nova ActionKind entrar sem mapeamento.
#
# mark_conflito → reviewer: MARK_CONFLITO só é emitido para PRs em review
#   detectados como CONFLICTING (domínio do cron reviewer, que já inspeciona
#   PRs em review e é o único que busca pr_mergeable).
# dispatch_conflict_resolver → conflito: age em issues que já carregam
#   crewflow:conflito (domínio do cron de conflito, junto do dispatch_rework).
_STAGE_ACTIONS = {
...
def _run_stage(ctx: object, stage: str) -> None:
    ...
    dispatch_reworks: list = []
    mark_conflitos: list = []          # ScanResult — issues para marcar crewflow:conflito
    conflict_resolvers: list = []      # (repo, issue) — cron de conflito resolve rebase
    needs_human: list = []
    blocked_bypass: list = []
    rebranded: list = []
    merge_prs: list = []
    ...
    for result in scan_results:
        ...
        # Busca o SHA do HEAD do PR (detecção de push pós-review) e a
        # mergeability (detecção de conflito de merge → crewflow:conflito).
        # Espelha o run() monolítico: para QUALQUER issue em review lê o PR,
        # pois pr_mergeable é pré-condição do MARK_CONFLITO — sem essa busca o
        # executor nunca emite MARK_CONFLITO no modo por-estágio (regressão #87).
        pr_head_sha: str | None = None
        pr_mergeable: str | None = None
        if result.current_state is State.REVIEW:
            import contextlib
    ...
    if not any([spec_invalid, dispatch_devs, dispatch_reviewers, dispatch_reworks,
                mark_conflitos, conflict_resolvers,
    ...
    if stage == _STAGE_DEV:
        ...
        # ── Aplica crewflow:conflito nas PRs com conflito de merge detectado ──
        # MARK_CONFLITO é emitido durante o review (PR CONFLICTING); o cron de
        # conflito depois localiza a branch e resolve o rebase.
        if mark_conflitos:
            for result in mark_conflitos:
                try:
                    _repo_mc = result.item.key.split("/issues/")[0].replace("https://github.com/", "") or (repos[0] if repos else "")
        ...
        # ── Resolve conflito de merge: rebase na branch existente ────────────
        # DISPATCH_CONFLICT_RESOLVER age em issues já marcadas crewflow:conflito;
        # localiza a branch feat/issue-N e despacha a sessão de resolução (conflict.md).
        for repo, issue in conflict_resolvers:
            issue_number = issue["number"]
            if not auto:
                ctx.notify(  # type: ignore[attr-defined]
                ...
                continue
            if _conflict_resolver_has_active(repo, issue_number):
                logger.info(
            ...
            branch_cr = f"feat/issue-{issue_number}"
            pr_number_cr: int | None = None
            try:
                import subprocess as _sp_cr
                _pr_cr_res = _sp_cr.run(
                    ["gh", "pr", "list", "--repo", repo, "--head", branch_cr,
                     "--state", "open", "--json", "number"],
                    capture_output=True, text=True, timeout=15, check=False,
                )
