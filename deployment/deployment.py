...
_DEV_PROMPT_FALLBACK = (
    ...
    "6. VALIDAÇÃO OBRIGATÓRIA E BLOQUEANTE — rode os MESMOS checks do CI DENTRO do worktree, "
    "ANTES de abrir a PR. Só prossiga para o passo 7 se TODOS passarem (verde). Para o kirocrew-flow:\n"
    "   ```bash\n"
    ...
    "   ```\n"
    "   Se o repo alvo NÃO for o kirocrew-flow, use os comandos de lint/type-check/test daquele projeto — "
    "descubra via `README.md`, `Makefile`/`Makefile.*`, `pyproject.toml`/`package.json`/`.github/workflows/*.yml`. "
    "NÃO presuma; se não houver checks definidos, rode ao menos o build/test padrão do projeto.\n"
    "   Se QUALQUER check falhar e você não conseguir corrigir dentro deste one-shot: NÃO abra a PR, "
    "marque `crewflow:blocked`, comente o erro na issue e ENCERRE.\n"
...
_REWORK_PROMPT_FALLBACK = (
    ...
    "5. VALIDAÇÃO OBRIGATÓRIA E BLOQUEANTE — rode os MESMOS checks do CI DENTRO do worktree, "
    "ANTES do push. Só prossiga para o passo 6 se TODOS passarem (verde). Para o kirocrew-flow:\n"
    "   ```bash\n"
    "   python3 -m ruff check flow/\n"
    "   python3 -m mypy flow/ --ignore-missing-imports\n"
    "   python3 -m pytest flow/tests/ --cov=flow --cov-fail-under=75\n"
    ...
    "descubra via `README.md`, `Makefile`/`Makefile.*`, `pyproject.toml`/`package.json`/`.github/workflows/*.yml`. "
    "NÃO presuma; se não houver checks definidos, rode ao menos o build/test padrão do projeto.\n"
    "   Se QUALQUER check falhar e você não conseguir corrigir dentro deste one-shot: NÃO faça commit/push "
    "(não atualize a PR), marque `crewflow:blocked`, comente o erro na PR e ENCERRE.\n"
    "   `gh issue edit {{issue_number}} --repo {{repo}} --add-label \"crewflow:blocked\" --remove-label \"crewflow:running\"`\n"
