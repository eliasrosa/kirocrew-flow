...
for f in base dev reviewer review_approved qa_approved conflict rework qa_notify qa_refused; do
...
echo "             every=300)"
echo ""
echo "    # Cron review-approved — merge squash pós-review (flow:review-approved -> flow:qa-waiting)"
echo "    cron_add(name=\"flow-review-approved\","
echo "             script=\"~/.kiro/crew/crons/deployment/flow/review_approved.py:run\","
echo "             every=120)"
echo ""
echo "    # Cron qa-approved — merge final pós-QA (flow:qa-approved -> flow:done)"
