"""kiro-esteira — vigia de issues `ready` + motor de execução one-shot.

Cron de SCRIPT do Kiro Crew (sem LLM, zero token no polling). Bate no GitHub via
`gh` em cada repo configurado, e para cada issue nova com label `ready` (sem
`crew: in progress`):
...
Registro (uma vez):
    cron_add(name="esteira", script="~/.kiro/crew/crons/esteira.py:run", every=600)

Config: ~/.kiro/crew/crons/esteira.config.yaml (copie de config.example.yaml).
"""
import glob
import json
import os
...
_CONFIG_CANDIDATES = [
    os.path.join(_HERE, "esteira.config.yaml"),
    os.path.expanduser("~/.kiro/crew/crons/esteira.config.yaml"),
]
...
def _load_config():
    ...
    if not path:
        raise RuntimeError(
            ...
            "esteira.config.yaml ao lado do script."
        )
    try:
...
def _ready_issues(repo):
    out = subprocess.run(
        ["gh", "issue", "list", "--repo", repo, "--state", "open",
         "--label", "ready", "--json", "number,title,url,labels", "--limit", "30"],
    ...
    for i in issues:
        ...
        if "crew: in progress" in names:
            continue
        i["_has_hold"] = "hold" in names
        filtered.append(i)
    return filtered
...
def _dispatch_prompt(repo, issue, cfg):
    short = repo.split("/")[-1]
    hold = (" (issue tem label `hold`: NÃO mergeie, deixe o PR pro humano revisar)"
            if issue.get("_has_hold") else "")
    ...
    return (
        ...
        "comente, marque `crew: needs-human`, avise e ENCERRE.\n"
        f"3. Marque `crew: in progress`. NÃO faça `git clone`. Use o clone em `{dev_root}/{short}` "
        ...
        "5. Valide localmente (build/testes).\n"
        f"6. Abra PR com 'Closes #{issue['number']}'{hold}; mergeie via squash quando verde se "
        "auto-merge permitido.\n"
        f"7. Ao terminar: {notify_step}LIMPE as labels de fluxo (ready/crew: in progress), "
        "e ENCERRE.\n"
        f"{vault_step}\n"
        "REGRAS CRÍTICAS:\n"
        "- UMA passada. Terminou, acabou. NÃO fique verificando, NÃO entre em loop.\n"
        "- Se algo bloquear, marque `blocked`/`crew: needs-human`, avise, e pare.\n"
        "------------------------------------------"
    )
...
def run(ctx):
    ...
    if auto and disparadas:
        ...
        ctx.notify(
            f"kiro-esteira: disparei sessão(ões) one-shot pra issue(s) `ready`. Avise o dono{vm} "
            f"que o disparo aconteceu.\n{linhas}{extra}"
        )
    elif auto and adiadas:
        fila = "\n".join(f"  - {r}#{i['number']}: {i['title']}" for r, i in adiadas)
        ctx.notify(f"kiro-esteira: {len(adiadas)} issue(s) `ready` na fila (limite cheio).{vm}\n{fila}")
    elif achados:
        blocos = [f"{r}:\n" + "\n".join(f"  - #{i['number']}: {i['title']} ({i['url']})"
                  for i in ns) for r, ns in achados]
        ctx.notify(
            f"kiro-esteira (Fase 1): há issue(s) `ready` esperando. Avise o dono{vm} com o TL;DR.\n"
            + "\n".join(blocos)
        )
