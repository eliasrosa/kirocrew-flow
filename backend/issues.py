...
chamada leve ao ``gh`` (mockável), degradando para ``0`` quando indisponível.
"""
...
def _age_min_from_gh(repo: str, number: int) -> int:
    """Calcula minutos desde a criação da issue via gh CLI (mockável).

    Vive em ``backend/`` (não em ``flow/``) para não tocar o motor. Degrada
    para ``0`` em qualquer falha — nunca propaga erro nem bloqueia a listagem.
    """
...
def _minutes_since_iso(iso_ts: str | None) -> int:
    """Minutos decorridos desde um timestamp ISO-8601, ou 0 se inválido."""
    if not iso_ts:
        return 0
    try:
...
def _age_min(conn: object, key: str, repo: str, number: int) -> int:
    """Idade em minutos no estágio atual.

    Preferência: ``running_since`` do cache (zero-token). Se ausente, faz uma
    chamada leve ao ``gh`` pelo ``createdAt``. Degrada para ``0``.
    """
...
def _card(item: dict[str, Any], project: str) -> dict[str, Any]:
    """Monta o card no formato exato esperado pela UI (sem age_min ainda)."""
